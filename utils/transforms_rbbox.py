...
def custom_gt_mask_bp_obbs(gt_masks, with_module=True):
...
def if_zero_area(label):
    """Check if the area of label is zero"""
    if ((label[0] == label[2] and label[1] == label[3]) or (
            label[0] == label[4] and label[1] == label[5])
            or (label[0] == label[6] and label[1] == label[7]) or (
                    label[2] == label[4] and label[3] == label[5])
            or (label[2] == label[6] and label[3] == label[7]) or (
                    label[4] == label[6] and label[5] == label[7])):
        return True
    return False
