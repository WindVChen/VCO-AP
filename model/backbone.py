...
class baseline(IHModelWithBackbone):
    def __init__(self, args, ocr=64):
        base_config = {'model': DeepImageHarmonization,
                       'params': {'depth': 7, 'batchnorm_from': 2, 'args': args}}

        params = base_config['params']

        backbone = HRNetV2(args, ocr=ocr)
        ...
        super(baseline, self).__init__(base_model, backbone, False, 'sum', args=args)
    ...
    def __init__(
        ...
        if pretained:
            self.load_pretrained_weights(
                r"pretrained/hrnetv2_w18_imagenet_pretrained.pth")
